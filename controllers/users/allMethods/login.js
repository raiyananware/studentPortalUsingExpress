...
const login = async (req, res) => {
  ...
  try {
    ...
    if (match) {
      ...
      res.json({ msg: "User Logged In", token: "Bearer " + jwtToken, userId:tempUserData.id });
    } else {
  ...
  res.status(500).json({msg:"You are InActive Please ask the administrator to activate yourself"})
}
}
else{
